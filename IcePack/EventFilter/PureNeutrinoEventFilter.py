...
@author: cyan.jo
Summary:
Filters events to retain only those involving genuine neutrinos.

(1)Checks the `is_neutrino` flag or equivalent metadata in the event table.
(2)Removes background noise or cosmic-ray–like events from the dataset.
(3)Often used early in the filtering pipeline to reduce non-neutrino contamination.
...
He generated lists of event numbers of the events that don't contain any photoelectrons from non-neutrino sources.
...
Special thanks to Janni for providing the event numbers of the pure neutrino events from I3 files :D
...
class PureNeutrinoEventFilter(EventFilter):
    def __init__(
        ...
        self.pure_nu_specifier_dir = (
            "/lustre/hpc/project/icecube/HE_Nu_Aske_Oct2024/clean_events_dict/"
        )
        super().__init__(
    ...
    def _set_valid_event_nos(self):
        ...
        if not relevant_csvs:
            self.logger.warning(
                f"No matching CSV files found for {self.subdir_no}/{self.part_no}"
            )
        ...
        valid_indices = [
            i
            for i, (rid, eid) in enumerate(zip(run_ids, event_ids))
            if (rid, eid) in pure_neutrino_events
        ]

        if not valid_indices:
            self.logger.warning(
                f"No valid events in {self.subdir_no}/{self.part_no}. Skipping filtering."
            )
            return None

        filtered_table = truth_table.take(valid_indices)
        self.valid_event_nos = set(
            filtered_table.column("event_no").to_pylist()
        )
        self.logger.info(
            f"Extracted {len(self.valid_event_nos)} valid events based on pure neutrino events."
        )
    ...
    def _get_relevant_csv_files(self, truth_table: pa.Table) -> list:
        ...
        relevant_files = [
            file
            for file_min, file_max, file in file_ranges
            if file_min <= max_id and file_max >= min_id
        ]

        self.logger.info(
            f"Selected {len(relevant_files)} CSV files covering event range [{min_id}, {max_id}]"
        )
    ...
    def _get_event_specifier_files(self) -> list:
        ...
        file_ranges = [
            ...
            for f in files
            if f.startswith("clean_event_ids_") and f.endswith(".csv")
        ]

        file_ranges.sort(key=lambda x: x[0])
        self.logger.info(
            f"Loaded {len(file_ranges)} event specifier files from {csv_path}"
        )
        return file_ranges
    ...
    def _load_pure_neutrino_events(self, csv_files: list) -> set:
        ...
        self.logger.info(
            f"Loaded {len(pure_neutrino_events)} valid (RunID, EventID) pairs from CSV files"
        )
