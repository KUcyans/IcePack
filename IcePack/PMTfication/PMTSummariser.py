...
class PMTSummariser:
    ...
    def _get_PMTfied_pa(self) -> pa.Table:
        ...
        for event_no, strings_doms_pulses in events_doms_pulses.items(): # event level loop
            ...
            if self.summary_mode == SummaryMode.SECOND or self.summary_mode == SummaryMode.EQUINOX or self.summary_mode == SummaryMode.SANKTHANS:
    ...
    def _process_DOM(self, pulses: np.ndarray, 
        ...
        if self.summary_mode == SummaryMode.SECOND:
            dom_data = np.hstack((dom_data, [eccentricity_PCA, aspect_contrast_PCA, hypotenuse])) # 32 + 3 = 35
        elif self.summary_mode == SummaryMode.EQUINOX:
            t_max_q = self._get_time_at_highest_charge_pulse(pulses, how_many=1)
            t_max_q_bottom = self._get_time_at_highest_charge_pulse_bottom(pulses)
            ...
            dom_data = np.hstack((dom_data, 
                                ...
                                [eccentricity_PCA, aspect_contrast_PCA, hypotenuse])) # 32 + 3 + 2 +3 = 40
            ...
            t_max_q1, t_max_q2 = self._get_time_at_highest_charge_pulse(pulses, how_many=2)
            t_max_q_bottom = self._get_time_at_highest_charge_pulse_bottom(pulses)
            Q_halftime = self._get_accumulated_charge_in_the_first_half(pulses)
            dom_data = np.hstack((dom_data,                                         # 32
                                [q_max1, q_max_2, q_max_bottom,                     # 3
                                t_max_q1, t_max_q2, t_max_q_bottom, Q_halftime],    # 4
                                [eccentricity_PCA, aspect_contrast_PCA, hypotenuse])) # 3
            # 32 + 3 + 4 + 3 = 42
    ...
    def _get_time_at_highest_charge_pulse(self, pulses: np.ndarray, how_many: int) -> np.ndarray:
        _fillIncomplete = -1
    ...
    def _get_time_at_highest_charge_pulse_bottom(self, pulses: np.ndarray) -> float:
        _fillIncomplete = -1
        if pulses.shape[0] < 6:
    ...
    def _get_max_charge_bottom(self, pulses: np.ndarray) -> float:
        ...
        if pulses.shape[0] < 4:
            ...
            max_charge_bottom = np.max(pulses[:, self.dom_charge_idx][bottom_half_mask])
